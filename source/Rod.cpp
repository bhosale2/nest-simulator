...
Rod::Rod(const int _n,
         vector<Vector3> _x,
         vector<Vector3> _v,
         vector<Matrix3> _Q,
         vector<Vector3> _w,
         vector<REAL> _l0,
         vector<Vector3> _intrinsic_k0,
         vector<Vector3> _intrinsicShearStrain0,
         const vector<REAL> _m,
         vector<REAL> _V,
         const REAL _density,
         vector<Matrix3> _J0,
         vector<Matrix3> _B,
    ...
    //—————————————
    D0 = Matrix3(0.0e-5, 0.0, 0.0, 0.0, 0.0e-5, 0.0, 0.0, 0.0, 1.0e-3);
    ...
    // members added specially for nest: check Rod.h for meanings
    cord = 0.0;
    timer = 1;
    TopHeight = 0.0;
    base_stress = 0.0;
...
void Rod::computeShearStrain0() {
    ...
    v_a_times_b_divide_c_minus_d_equal_c(
        ...
        shearStrain0);  // shearStrain0 = Q * d/dS{r} - onez = Q * edge / l0 -
                        // onez; (note: edge must be normalized by l0 and not l
                        // if you want to capture extension! tangent=e/l0)
...
void Rod::computeCurvature0() {
    ...
    // kz*d3, and with respect to the reference configuration k = d/dS{Q}, that
    // is that the derivative is taken by dividing for d0!!
    //
    // Discrtetization:
    //		k[i] = log( Q[i]*Q^T[i-1] ) / d0
    //
    // The next three lines are equivalent to k0 = vLog(vRotDiff(Q)) / d0 -->
    // but no memory allocation (in-place)
    ...
    v_a_times_b_equal_c(tempVV3_nminus, 1.0 / dt, k0_rate);
}

void Rod::computeInternalShearForces0(const REAL time) {
    // Compute the internal shear forces n in the material frame and with
    // respect to the reference configuration! It is important to notice that
    // here we use a linear contitutive relation based on the definition of
    // engineering stress-strain curve, opposite to the true strain-stress
    // curve! Engineering stress-strain --> stress measured experimentally as
    // stress = F/(E*A0) where A0 is the area PRIOR applying the load True
    // stress-strain --> stress measured experimentally as stress = F/(E*A)
    // where A is the area AFTER applying the load Therefore since we use the
    // engineering stress-strain, we do NOT use the current S = S0/e, but we use
    // the S0 PRIOR the load

    // Strain rate damping factor
    REAL factor = (time - 0.01) * 200000.0;
    ...
    v_a_times_b_equal_c(DS_0, shearStrain_rate, tempVV3_n2);
    v_a_plus_b_equal_c(tempVV3_n, tempVV3_n2, shearInternalForces0);

    // cout<<tempVV3_n<<" "<<"ela"<<endl;
    // cout<<tempVV3_n2<<" "<<"Dam"<<endl;
}
...
void Rod::computeInternalBendingTorques0(const REAL time) {
    ...
    v_a_minus_b_equal_c(k0, intrinsic_k0,
                        kDiff0);  // kDiff0 = k0 - intrinsic_k0;
    v_a_times_b_equal_c(
        B0, kDiff0, tempVV3_nminus);  // bendingInternalTorques0 = B0 * kDiff0;
    v_a_times_b_equal_c(DB_0, k0_rate, tempVV3_nminus2);
    v_a_plus_b_equal_c(tempVV3_nminus, tempVV3_nminus2,
                       bendingInternalTorques0);
}
...
void Rod::computeInternalForces() {
    ...
    //stress-strain
    // realtion (see above) 					if we were to be using the true
    // stress-strain relationsin we should apply the rescaling n = n0/e but this
    // introduce an asymmetry in the load-strain relation
    // which makes the system unstable (this is why we stick to the engineering
    //definition)
    //
    // Equalities:
    // 		ds = e*dS
...
void Rod::computeInternalTorques() {
    ...
    // external torques) in the material frame of reference and wihrespect to
    ...
    // 		s = actual configuration arc-lenght
    //
    // Divergence of internal torques plus the tangent cross internal shear
    // forces gives the torques (per unit length!) acting on the infinitesimal
    // element of length ds, and expressed in the lab frame of reference:
    ...
    // J*w 		d/dt{J*w} = d/ds{ B*d/ds{Q} }*ds + (d/ds{Q} x B*k) + (Q*d/ds{r}
    // x n)*ds - w x J*w
    //
    // From actual configuration (function of s) to reference configuration
    // (function of S):
    ...
    // multiplied by ds, because it 																										is already
    // contained in its definition J = rho*I*ds (where I is the area moment here)
    // d/dt{J0/e*w} = (1/e)*d/dS{ (B0/e2)*(1/e)*d/dS{Q} }*e*dS +
    //( (1/e)*d/dS{Q} x B0/e2*(1/e)*d/dS{Q}
    //)*e*dS + 						(Q*(1/e)*d/dS{r} x n0)*e*ds - 						w x J0/e*w
    //J0*d/dt{w/e} = d{B0*k0/e3} + (k0 x B0*k0/e3)*dS + Q*d{r} x n0 - w x
    // (J0/e)*w
    //
    ...
    // Term relative to the transport of angular velocity in the angular
    // momentum balance due to the use of material frame of reference (J0*w/e) x
    // w
    transportW = (J0 * w / e) * w;
}
...
void Rod::computeDampingForces() {
    // This damping model is in the spirit of Coulomb dissipation model
    // (f=-nu*v), but accounts only for internal dissipation by using relative
    // velocities between elements. This way rigid body motions are not
    ...
    // exponentially (make sure you are setting the time in the verlet
    // integrator!)
    REAL nuNow = (relaxationNu == 0.0 || nu == 0.0)
                     ? nu
                     : nu * exp(-time / relaxationNu);
    ...
    for (unsigned int i = 0; i < dampingForces.size(); i++) {
        const REAL factor =
            (i == 0 || i == (dampingForces.size() - 1)) ? 0.5 : 1.0;
...
void Rod::computeAllInternalResultingForcesAndTorques() {
    ...
    computeDampingForces();  // in-place————replaced by the internal
                             // dissipaition
...
void Rod::computeSelfCollisionForces() {
    ...
    for (int i = 0; i < (int)n; i++) {
        ...
        // distance ~= PI*R of this one b/c the other energy terms already
        // account for the local curvature bounded regime
        ...
        for (int j = i - skip; j > -1; j--) {
            ...
            if ((x1 - x2).length() <= (sum_r1r2 + l[i] + l[j])) {
                ...
                // Remenber that the first and last points have half the mass!
                // Hence the strange coefficient in fron of cForce
                selfCollisionForces[i] -= ((i == 0) ? 0.5 : 1.0) * cForce;
                selfCollisionForces[i + 1] -=
                    ((i + 1 == n) ? 0.5 : 1.0) * cForce;
                selfCollisionForces[j] += ((j == 0) ? 0.5 : 1.0) * cForce;
                selfCollisionForces[j + 1] +=
                    ((j + 1 == n) ? 0.5 : 1.0) * cForce;
            }
        }
    }
}
...
void Rod::applyStaticFrictions() {
    ...
    for (unsigned int i = 0; i < totalInternalForces.size(); i++) {
        ...
        // plane) direction
        REAL projectionSign = ((totalInternalForces[i] + externalForces[i]) %
                               maxFrictionDirection);

        Vector3 projection = projectionSign * maxFrictionDirection;

        // If total forces are pushing forward, so (projectionSign>0.0) then the
        // max friction is given by staticFrictionsAxialForceForward, otherwise
        // it is given by staticFrictionsAxialForceBackward
        ...
        Vector3 forceStatic = -projection.unitize() *
                              min(projection.length(), maxFriction.length());
    ...
    /*
            // This part compute the torque induced by the friction at the
    bottom
    ...
    (-r[i]*normalPlaneDirection); externalTorques[i] += Q[i]*(arm *
    tempVV3_n[i]);
    }
     */
    ...
    // ensure robustness, whenever one mass point for numerical reasons one
    // might consider to take the max of the mass point static elements
    ...
    // The point mass velocities are translated into the velocities of
    ...
    // Compute force generated by current torque on the surface and store them
    // in the temporary container tempVV3_nplus
    for (unsigned int i = 0; i < totalInternalTorques.size(); i++) {
        // Rod surface static
        ...
        const Vector3 torqueInLabFrame =
            materialFrame.T() * torqueInMaterialFrame;
        ...
        const Vector3 forceElementRolling =
            (tempVV3_n[i] % maxRollFrictionDirection) *
            maxRollFrictionDirection;
        ...
        // rolling forces and torques neglected for nest - Yashraj
        // Correct forces
        ...
        // when the right conditions are satisfied. It is not necessary and it
        // is actually potentially harful, since it abruptly modifies the
        // dynamics of an element inducing strong reaction forces. Nevertheless
        // I leave this piec of code here, just in case, but I do not advice to
        // use it
        // if ( (forceNoSlip.length() - maxRollFriction.length() ) <=
        // Tolerance::tol() )
        //{
        //	const Vector3 translationVelInRollingDirection = (overallVel[i]
        //% maxRollFrictionDirection)*maxRollFrictionDirection; 	const
        //Vector3 rotationalVelInRollingDirection =
        ...
        //-factor*slipVelInRollingDirection/2.0; 	const Vector3
        //slipVelCorrection = -slipVelInRollingDirection/2.0;

        //	const Vector3 correction_v_i =
        //((i>0)?0.5:1.0)*slipVelCorrection; 	const Vector3
        //correction_v_i_plus =
...
void Rod::applyKineticFrictions() {
...
void Rod::reset() {
    ...
    fill(staticFrictionsNormalPlane_R.begin(),
         staticFrictionsNormalPlane_R.end(), zero);
...
void Rod::dumpPovray(string filePovray,
                     string fileData,
                     int colorindex,
                     REAL time) {
    // draw a cylinder
    if (colorindex == 0) {
        ...
        f << "texture\n";
        f << "{\n";
        f << "pigment{ color rgbt<0.827451,0.827451,0.827451,0.95> }\n";
        f << "finish { phong 1 }\n";
        f << "}\n";
...
void Rod::paint() {
    ...
    // Draw body top view
    {
        ...
        glBegin(GL_LINE_STRIP);
        {
            for (unsigned int i = 0; i < x.size(); i++)
                glVertex2f((0.5 / SPAN) * x[i].x + 0.25,
                           (0.5 / SPAN) * x[i].y + 0.75);
        }
    ...
    // Draw head top view
    {
        ...
        glBegin(GL_LINE_STRIP);
        {
            for (unsigned int i = 0; i < (unsigned int)min(5, (int)x.size());
                 i++)
                glVertex2f((0.5 / SPAN) * x[i].x + 0.25,
                           (0.5 / SPAN) * x[i].y + 0.75);
        }
    ...
    // Draw body front view
    {
        ...
        {
            for (unsigned int i = 0; i < x.size(); i++)
                glVertex2f((0.5 / SPAN) * x[i].y + 0.75,
                           (0.5 / SPAN) * x[i].z + 0.25);
        }
...
void Rod::draw_arrow(const float posx,
                     const float posy,
