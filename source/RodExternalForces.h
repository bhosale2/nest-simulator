...
// constant load is applied for iso friction validation
class isof_Load : public ExternalForces
{
public:
	const Vector3 F;
	const REAL omega;

	isof_Load(const Vector3 _F, const REAL _omega) : ExternalForces(), F(_F), omega(_omega) {}

	void applyForces(Rod &R, const REAL time)
	{
		if (time >= 0)
		{
		R.externalForces += F*omega*(time/0.1);

		// This is bacause, the first and last masses are half the others!
		R.externalForces.front() -= F*omega*(time/0.1)/2.0;
		R.externalForces.back() -= F*omega*(time/0.1)/2.0;	/* code */
		}
		//R.externalForces.front() += F*omega*0.5;
