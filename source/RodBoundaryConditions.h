...
//restraing a rod completely 
class RestrictfullBC : public RodBC
{
	const Vector3 RodOrigin;
	const Vector3 RodDirection;
	const REAL L0;
public:
	RestrictfullBC(const Vector3 _RodOrigin, const Vector3 _RodDirection, const REAL _L0): RodBC(), RodOrigin(_RodOrigin), RodDirection(_RodDirection), L0(_L0) {}

	void dirichlet(Rod& rod, const unsigned int step, const REAL dt, const REAL time)
	{
		// Completely fix the rod
		int n = rod.x.size();

		for(unsigned int i=0; i<rod.x.size(); i++)
		{		
			Vector3 temp = RodOrigin + i*L0*RodDirection/(n-1);
			rod.x[i].x = temp.x;
			rod.x[i].y = temp.y;
			rod.x[i].z = temp.z;
		}	
